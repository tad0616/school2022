<{if $logo_position!="slide" && $logo_cols!='' && $logo_enable|default:'1'=='1'}>
    <div id="logo-wrapper">
        ...
        <div id="logo-display" class="container-fluid <{if $logo_align|default:false}>d-flex <{$logo_align|default:''}><{/if}>">
            ...
            <{/if}>
            <div class="col-xl-<{if $logo_col.0|default:false}><{$logo_col.0}><{else}>auto<{/if}> text-start">
                <{if $logo_text=='1'}>
                    <h1>
                    <a href="<{$xoops_url}><{$smarty.server.DOCUMENT_URI}>?<{$logo_var|default:''}>=<{$smarty.get.$logo_var|default:''}>"><{$xoops_sitename|default:''}></a>
                    </h1>
